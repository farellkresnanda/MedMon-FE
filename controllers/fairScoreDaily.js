...
const processData = async (startDate, endDate, kategori, platform) => {
  ...
  console.info(
    `[INFO] Starting Daily processing for ${dates.length} dates with chunk size ${chunkSize}`
  );
...
const processFollowers = async (date, kategori, platform) => {
  ...
  if (updates.length) {
    ...
    console.info(
      `[SUCCESS] Daily Updated followers for ${updates.length} rows.`
    );
  } else {
...
const processActivities = async (date, kategori, platform) => {
  ...
  if (updates.length) {
    ...
    console.info(
      `[SUCCESS] Daily Updated activities for ${updates.length} rows.`
    );
  } else {
...
const processInteractions = async (date, kategori, platform) => {
  ...
  if (updates.length) {
    ...
    console.info(
      `[SUCCESS] Daily Updated interactions for ${updates.length} rows.`
    );
  } else {
...
const processResponsiveness = async (date, kategori, platform) => {
  console.info(
    `[INFO] Daily Processing responsiveness (from posts.responsiveness_post) for ${date}`
  );
  const [rows] = await connection.query(
    `
        SELECT fsd.list_id, fsd.username,
  ...
  if (updates.length) {
    ...
    console.info(
      `[SUCCESS] Daily responsiveness updated from posts.responsiveness_post for ${updates.length} rows.`
    );
  } else {
...
const processFinalFairScore = async (date, kategori, platform) => {
  ...
  if (!max || Object.values(max).every((v) => v === null)) {
    console.warn(
      "[WARN] Daily No max values found for FAIR score calculation."
    );
    return;
  }
  ...
  if (updates.length) {
    ...
    console.info(
      `[SUCCESS] Daily Final FAIR scores updated for ${kategori} on platform ${platform}.`
    );
  } else {
