...
const processData = async (startDate, endDate, kategori, platform) => {
  ...
  console.info(
    `[INFO] Monthly Starting processing for ${dates.length} dates with chunk size ${chunkSize}`
  );
...
const processMonthlyFollowers = async (date, kategori, platform) => {
  ...
  if (updates.length) {
    ...
    console.info(
      `[SUCCESS] Monthly Updated followers for ${updates.length} rows.`
    );
  } else {
...
const processMonthlyActivities = async (date, kategori, platform) => {
  ...
  if (updates.length) {
    ...
    console.info(
      `[SUCCESS] Monthly Updated activities for ${updates.length} rows.`
    );
  } else {
...
const processMonthlyInteractions = async (date, kategori, platform) => {
  ...
  if (updates.length) {
    ...
    console.info(
      `[SUCCESS] Monthly Updated interactions for ${updates.length} rows.`
    );
  } else {
...
const processMonthlyResponsiveness = async (date, kategori, platform) => {
  console.info(
    `[INFO] Monthly Processing responsiveness (from posts.responsiveness_post) for ${date}`
  );
  const [rows] = await connection.query(
    `
        SELECT fsm.list_id, fsm.username,
  ...
  if (updates.length) {
    ...
    console.info(
      `[SUCCESS] Monthly responsiveness updated from posts.responsiveness_post for ${updates.length} rows.`
    );
  } else {
...
const processMonthlyFairScore = async (date, kategori, platform) => {
  ...
  if (!max || Object.values(max).every((v) => v === null)) {
    console.warn(
      "[WARN] Monthly No max values found for FAIR score calculation."
    );
    return;
  }
  ...
  const updates = rows.map((row) => {
    ...
    const responsiveness_score =
      row.responsiveness / (max.max_responsiveness || 1);
  ...
  if (updates.length) {
    ...
    console.info(
      `[SUCCESS] Monthly Final FAIR scores updated for ${kategori} on platform ${platform}.`
    );
  } else {
