...
const savePost = async (post) => {
  ...
  connection.query(
    sql,
    [
      normalizeToString(post.client_account),
      normalizeToString(post.kategori),
      post.platform,
      post.user_id,
      post.unique_id_post,
      post.username,
    ...
    (err) => {
      if (err) {
        console.error(
          `❌ Error saving post ${post.unique_id_post}:`,
          err.message
        );
      } else {
...
const saveComment = async (comment) => {
  ...
  connection.query(
    sql,
    [
      normalizeToString(comment.client_account),
      normalizeToString(comment.kategori),
      comment.platform,
      comment.user_id,
      comment.username,
    ...
    (err) => {
      if (err) {
        console.error(
          `❌ Error saving comment on post ${comment.unique_id_post}:`,
          err.message
        );
      } else {
        console.log(
          `✅ Saved comment ${comment.comment_unique_id} on post ${comment.unique_id_post}`
        );
      }
    }
  );
...
const saveChildComment = async (child) => {
  ...
  connection.query(
    sql,
    [
      normalizeToString(child.client_account),
      normalizeToString(child.kategori),
      child.platform,
      child.user_id,
      child.username,
      child.unique_id_post,
      child.comment_unique_id,
      child.child_comment_unique_id,
      child.created_at,
      child.child_commenter_username,
      child.child_commenter_userid,
      child.child_comment_text,
      child.child_comment_like_count,
    ],
    (err) => {
      if (err) {
        console.error(
          `❌ Error saving child comment for ${child.comment_unique_id}:`,
          err.message
        );
      } else {
        console.log(
          `✅ Saved child comment ${child.child_comment_unique_id} for ${child.comment_unique_id}`
        );
      }
    }
  );
...
const saveLikes = async (likes) => {
  ...
  connection.query(
    sql,
    [
      normalizeToString(likes.client_account),
      normalizeToString(likes.kategori),
      likes.platform,
      likes.post_code,
      likes.user_id,
      likes.username,
    ...
    (err) => {
      if (err) {
        console.error(
          `❌ Error saving like on post ${likes.post_code}:`,
          err.message
        );
      } else {
        console.log(
          `✅ Saved like from ${likes.username} on post ${likes.post_code}`
        );
      }
    }
  );
...
const saveDataPostByKeywords = async (post) => {
  ...
  connection.query(
    ...
    (err) => {
      if (err) {
        console.error(
          `❌ Error saving keyword-post ${post.unique_id_post}:`,
          err.message
        );
      } else {
        console.log(
          `✅ Saved keyword-post ${post.unique_id_post} for keyword "${post.keywords}"`
        );
      }
    }
  );
