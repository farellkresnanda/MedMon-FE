...
const processDataFair = async (startDate, endDate, kategori, platform) => {
  console.info(
    `[INFO] Processing FAIR Score as JSON from ${startDate} to ${endDate} for kategori "${kategori}" & platform "${platform}"`
  );

  const totalDays = getDaysDiff(startDate, endDate);
  const finalDate = new Date(endDate).toISOString().split("T")[0];

  const rows = await getCombinedMonthlyData(
    startDate,
    endDate,
    totalDays,
    kategori,
  ...
  const result = rows.map((row) => {
    ...
    const responsiveness_score =
      row.responsiveness / (max.responsiveness || 1) || 0;

    const fair_score =
      ((followers_score * 2 +
        activities_score * 2 +
        interactions_score * 3 +
...
const getCombinedMonthlyData = async (
  startDate,
  endDate,
  totalDays,
  kategori,
