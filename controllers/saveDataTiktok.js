...
const saveUser = async (user) => {
  ...
  connection.query(
    ...
    (err, result) => {
      if (err) {
        console.error(
          `Error saving user ${user.username} to database:`,
          err.message
        );
      } else {
        console.log(
          `Saved user ${user.username}, ${user.kategori}, untuk platform ${user.platform} to database`
        );
      }
    }
  );
...
const savePost = async (post) => {
  ...
  try {
    ...
    console.log(
      `✅ Saved post ${post.unique_id_post} for ${post.username} in kategori ${post.kategori}`
    );
  } catch (error) {
    console.error(
      `❌ Error saving post ${post.unique_id_post} to database:`,
      error.message
    );
  }
};
...
const saveComment = async (comment) => {
  ...
  connection.query(
    ...
    (err, result) => {
      if (err) {
        console.error(
          `❌ Error saving comment ${comment.comment_unique_id} for ${comment.client_account}:`,
          err.message
        );
      } else {
        console.log(
          `✅ Saved comment ${comment.comment_unique_id} for ${comment.client_account} in kategori ${comment.kategori}`
        );
      }
    }
  );
...
const saveChildComment = async (childComment) => {
  ...
  connection.query(
    sql,
    [
      childComment.client_account,
      childComment.kategori,
      childComment.platform,
    ...
    (err, result) => {
      if (err) {
        console.error(
          `❌ Error saving child comment ID ${childComment.child_comment_unique_id} for ${childComment.client_account}:`,
          err.message
        );
      } else {
        console.log(
          `✅ Saved child comment ID ${childComment.child_comment_unique_id} for ${childComment.client_account} in kategori ${childComment.kategori}`
        );
      }
    }
  );
  ...
  connection.query(
    ...
    (err, result) => {
      if (err) {
        ...
        console.log(
          `✅ Saved post ${post.unique_id_post} for ${post.client_account} in kategori ${post.kategori}`
        );
      }
    }
  );
