...
router.post("/addDataUser", async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: `Data user berdasarkan kategori ${kategori} dan platform ${platform} berhasil disimpan ke dailyFairScores.`,
    });
  } catch (error) {
  ...
  try {
    ...
    for (const cat of categories) {
      for (const plat of platforms) {
        tasks.push(
          fairScores
            .processDataFair(start_date, end_date, cat, plat)
            .then((data) => {
              allResults.push({
                kategori: cat,
                platform: plat,
    ...
    res.json({
      success: true,
      message: `Data berhasil diproses untuk kategori: ${categories.join(
        ", "
...
router.post("/processData", async (req, res) => {
  try {
    ...
    const categories = kategori
      ? [kategori]
      : await getCategoriesFromListAkun();
    console.info(`Processing categories: ${categories.join(", ")}`);
    ...
    for (const cat of categories) {
      for (const plat of platforms) {
        monthlyTasks.push(
          fairScoreMonthly.processData(start_date, end_date, cat, plat)
        );
      }
    }

    console.info(
      `[INFO] Starting Monthly FAIR Score for ${kategori} on ${platform}...`
    );
    await Promise.all(monthlyTasks);
    console.info(
      `[SUCCESS] Monthly FAIR Score for ${kategori} on ${platform} completed.`
    );
    ...
    for (const cat of categories) {
      for (const plat of platforms) {
        dailyTasks.push(
          fairScoreDaily.processData(start_date, end_date, cat, plat)
        );
      }
    }

    console.info(
      `[INFO] Starting Daily FAIR Score for ${kategori} on ${platform}...`
    );
    await Promise.all(dailyTasks);
    console.info(
      `[SUCCESS] Daily FAIR Score for ${kategori} on ${platform} completed.`
    );
...
router.post("/update-followers", async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i <= daysRange; i++) {
      ...
      for (const [username, posts] of Object.entries(postsByUser)) {
        ...
        const newFollowers = Math.max(
          0,
          Math.floor(curr.followers * (1 - growthRate))
        );
    ...
    res.json({
      message: "Selesai update followers berdasarkan performa postingan.",
    });
  } catch (err) {
    console.error("Update gagal:", err);
    res
      .status(500)
      .json({ message: "Gagal update data postingan.", error: err.message });
  }
});
