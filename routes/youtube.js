...
router.get("/update-followers-kdm", async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT * FROM posts WHERE FIND_IN_SET("kdm", kategori) AND platform = "Youtube"'
    );
...
router.get("/getData", async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      'SELECT * FROM listakun WHERE platform = "Youtube" AND FIND_IN_SET(?, kategori)',
      [kategori]
    );
...
router.get("/getPost", async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      'SELECT * FROM users WHERE platform = "Youtube" AND FIND_IN_SET(?, kategori)',
      [kategori]
    );
  ...
  try {
    ...
    if (!Array.isArray(unique_id_post) || unique_id_post.length === 0) {
      return res
        ...
        .json({
          error:
            "Invalid unique_id_post format. It should be a non-empty list.",
        });
    }

    console.log(
      `🚀 Starting to fetch main comments for ${unique_id_post.length} posts...`
    );
    ...
    res
      ...
      .json({
        message:
          "✅ Data getComment and getChildComment processed successfully.",
      });
  } catch (error) {
...
router.post("/getCommentv3", async (req, res) => {
  try {
    ...
    console.log(
      `🚀 Starting to fetch comments for ${unique_id_post.length} posts...`
    );
    ...
    for (const code of unique_id_post) {
      ...
      try {
        ...
        console.log(
          `✅ Comments for post ${code} have been fetched and saved.`
        );
      } catch (err) {
        console.error(
          `❌ Error fetching comments for post ${code}:`,
          err.message
        );
      }
    }
