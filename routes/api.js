...
router.post("/prosesPerformaKonten", async (req, res) => {
  try {
    ...
    if (totalRows === 0) {
      console.warn(
        "⚠️ Tidak ada data yang diperbarui dalam rentang tanggal tersebut."
      );
      return res.json({
        code: 200,
        status: "OK",
    ...
    const progressBar = new cliProgress.SingleBar(
      {
        format:
          "📊 Progress [{bar}] {percentage}% | {value}/{total} posts processed",
      },
...
router.get("/getInteractionsRanking", async (req, res) => {
  try {
    ...
    res.status(500).json({
      code: 500,
      status: "ERROR",
      message: "Failed to fetch interactions ranking",
    });
  }
});
...
router.get("/getAllPost", async (req, res) => {
  try {
    ...
    const orderBy = allowedOrderByFields.includes(req.query["orderBy"])
      ? req.query["orderBy"]
      : "performa_konten";
    ...
    const { kategori, platform, start_date, end_date, username, media_name } =
      req.query;
    ...
    const [percentileRows] = await db.query(percentileQuery, [
      kategori,
      platform,
      start_date,
      end_date,
    ...
    console.info(
      `Calculated Percentiles (Grouped): 10% = ${percentile10}, 90% = ${percentile90}`
    );
    ...
    res
      .status(500)
      .json({ code: 500, status: "ERROR", message: "Failed to fetch posts" });
  }
});
...
router.get("/getAllUsername", async (req, res) => {
  try {
    ...
    const queryParams = [req.query["kategori"], req.query["platform"]];
  ...
  try {
    ...
    if (!username || !month) {
      return res.status(400).json({
        code: 400,
        status: "ERROR",
        message: "username dan month wajib diisi.",
      });
    }

    const summary = await generateFairSummary(
      username,
      month,
      kategori,
      platform
  ...
  try {
    ...
    if (!username || !month) {
      return res.status(400).json({
        code: 400,
        status: "ERROR",
        message: "Parameter username dan month wajib diisi.",
      });
    }
