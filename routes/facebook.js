...
const trackRequests = async () => {
  ...
  if (requestCount >= maxRequestsPerMinute) {
    console.log(
      `Reached ${maxRequestsPerMinute} requests. Resting globally for ${
        delay / 1000
      } seconds...`
    );
  ...
  const queue = async.queue(async (item, callback) => {
    try {
      ...
      if (threadRequestCount >= threadRequestLimit) {
        console.log(
          `Thread reached ${threadRequestLimit} requests. Resting thread for ${
            threadRestTime / 1000
          } seconds...`
        );
...
router.get("/update-followers-kdm", async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT * FROM posts WHERE FIND_IN_SET("kdm", kategori) AND platform = "Facebook"'
    );
...
router.get("/getData", async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      'SELECT * FROM listAkun WHERE platform = "Facebook" AND FIND_IN_SET(?, kategori)',
      [kategori]
    );
    ...
    await processQueue(rows, async (row) => {
      try {
        ...
        console.log(
          `Data for user ${row.username} has been fetched and saved.`
        );
      } catch (error) {
        console.error(
          `Error fetching data for user ${row.username}:`,
          error.message
        );
      }
    });
...
router.get("/getPost", async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      'SELECT * FROM users WHERE platform = "Facebook" AND FIND_IN_SET(?, kategori)',
      [kategori]
    );
    ...
    await processQueue(rows, async (row) => {
      ...
      console.log(
        `Posts for user ${row.username} have been fetched and saved.`
      );
    });
...
const getDateRange = async () => {
  try {
    const [rows] = await db.query(
      "SELECT startDate, endDate FROM settings WHERE id = 1"
    );
...
router.get("/getComment", async (req, res) => {
  ...
  try {
    ...
    if (!dateRange) {
      return res
        .status(500)
        .json({ message: "Gagal mendapatkan rentang tanggal dari database." });
    }
    ...
    const [mainComments] = await db.query(mainCommentQuery, [
      kategori,
      startDate,
      endDate,
    ]);
    ...
    await processQueue(mainComments, async ({ unique_id_post }) => {
      ...
      const { user_id, username, comments, client_account, platform } =
        userRows[0];
      ...
      if (comments > 0) {
        try {
          ...
          console.log(
            `✅ Comments for post ${unique_id_post} have been fetched and saved.`
          );
        } catch (err) {
          console.error(
            `❌ Error fetching comments for post ${unique_id_post}:`,
            err.message
          );
        }
      } else {
    ...
    const [childComments] = await db.query(childCommentQuery, [
      kategori,
      startDate,
      endDate,
    ]);
    ...
    await processQueue(
      ...
      async ({
        ...
        console.log(
          `🔍 Fetching child comments for comment ID: ${comment_unique_id} on post: ${unique_id_post}...`
        );
        ...
        if (child_comment_count > 0) {
          try {
            ...
            console.log(
              `✅ Child comments for comment ID ${comment_unique_id} on post ${unique_id_post} have been fetched and saved.`
            );
          } catch (err) {
            console.error(
              `❌ Error fetching child comments for comment ID ${comment_unique_id}:`,
              err.message
            );
          }
        } else {
          console.log(
            `ℹ️ No child comments for comment ID ${comment_unique_id}.`
          );
        }
      }
    );
    ...
    res.send(
      "✅ Data getComment and getChildComment for all users have been fetched and saved."
    );
  } catch (error) {
    console.error(
      "❌ Error executing getComment and getChildComment:",
      error.message
    );
    res.status(500).json({
      message:
        "Terjadi kesalahan saat menjalankan proses getComment dan getChildComment.",
      error: error.message,
    });
  }
});
  ...
  try {
    ...
    await processQueue(rows, async (row) => {
      ...
      try {
        ...
        console.error(
          `Error fetching likes for post ${post_code}:`,
          err.message
        );
      }
    });
...
router.get("/getDataPostByKeywords", async (req, res) => {
  ...
  try {
    ...
    await processQueue(rows, async (row) => {
      ...
      console.log(
        `Posts for keywords ${row.keyword} have been fetched and saved.`
      );
    });

    res.send(
      "Data getDataPostByKeywords for all users have been fetched and saved."
    );
  } catch (error) {
    console.error("Error executing getDataPostByKeywords:", error.message);
    res
      .status(500)
      .send(`Error executing getDataPostByKeywords: ${error.message}`);
  }
});
...
router.post("/getCommentv2", async (req, res) => {
  ...
  if (!kategori || !unique_id_post) {
    return res.status(400).json({
      message: '❌ Parameters "kategori" and "unique_id_post" are required.',
    });
  }
