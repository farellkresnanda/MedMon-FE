...
const {
  calculateResponsivenessPerPost,
} = require("../controllers/calculateResponsivenessPerPost");
  ...
  try {
    ...
    if (!kategori || !platform || !start_date || !end_date) {
      return res.status(400).json({
        message: "Kategori, platform, start_date, dan end_date wajib diisi.",
      });
    }

    console.info("Received Export Request: ", {
      kategori,
      platform,
      start_date,
      end_date,
    ...
    console.info(
      `Calculated Percentiles: 10% = ${percentile10}, 90% = ${percentile90}`
    );
    ...
    res
      .status(500)
      .json({ code: 500, status: "ERROR", message: "Failed to export posts" });
  }
});
...
router.post("/exportAllComments", async (req, res) => {
  try {
    ...
    // 💾 Save and Send
    const filename = `comments_${kategori}_${platform}_${start_date}_to_${end_date}${
      username ? `_for_${username}` : ""
    }.xlsx`;
    res.setHeader(
      "Content-Type",
      "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
...
router.get("/exportFair", async (req, res) => {
  try {
    ...
    console.info("[INFO] Received Export Request: ", {
      kategori,
      platform,
      start_date,
      end_date,
    ...
    const [maxDateResult] = await db.query(maxDateQuery, [
      kategori,
      platform,
      start_date,
      end_date,
    ...
    if (!latestDate) {
      return res.status(404).json({
        message:
          "Tidak ada data yang ditemukan dalam rentang tanggal tersebut.",
      });
    }
    ...
    if (dataRows.length === 0) {
      return res
        .status(404)
        .json({ message: "Tidak ada data yang ditemukan untuk export." });
    }
