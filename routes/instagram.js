...
router.get("/getPost", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send(
        `✅ Posts ${PLATFORM} for category "${kategori}" have been fetched and saved.`
      );
  } catch (error) {
...
router.get("/getComment", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send(
        `✅ Comments and child comments ${PLATFORM} for category "${kategori}" have been fetched and saved.`
      );
  } catch (error) {
...
router.get("/getCommentByCode", async (req, res) => {
  ...
  if (!kategori || !url)
    return res
      .status(400)
      .json({ message: "❌ kategori and url parameter are required." });
  ...
  try {
    ...
    res.status(500).json({
      message: "❌ Error fetching comments by code.",
      error: error.message,
    });
  }
});
...
router.get("/getLikes", async (req, res) => {
  try {
    const [rows] = await db.query("SELECT * FROM posts WHERE platform = ?", [
      PLATFORM,
    ]);
...
router.get("/getDataPostByKeywords", async (req, res) => {
  ...
  try {
    ...
    await Promise.all(
      rows.map((row) =>
        getDataIg.getDataPostByKeyword(
          row.kategori,
          PLATFORM,
          row.client_account,
          row.keyword
    ...
    res
      .status(200)
      .send(
        `✅ Data post ${PLATFORM} by keywords fetched for category ${kategori}.`
      );
  } catch (error) {
...
router.post("/getPostDataByCode", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send(`✅ Posts by code fetched and saved for platform ${platform}.`);
  } catch (error) {
