...
router.get("/update-followers", async (req, res) => {
  ...
  try {
    await getDataTiktok.getDataFollowers(kategori, platform);
...
router.get("/getPost", async (req, res) => {
  ...
  try {
    ...
    await getDataTiktok.getDataPost(kategori, "TikTok", startDate, endDate);

    res
      .status(200)
      .send(
        `✅ Data ${platform} posts for category "${kategori}" have been fetched and saved.`
      );
  } catch (error) {
...
router.get("/getComment", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send(
        `✅ Comments and child ${platform} comments for category "${kategori}" have been fetched and saved.`
      );
  } catch (error) {
    console.error(
  ...
  try {
    ...
    await Promise.all(
      rows.map(async (row) => {
        ...
        console.info(
          `Posts for keyword ${row.keyword} have been fetched and saved.`
        );
      })
    );
    ...
    res
      .status(200)
      .send(
        `Data TikTok getDataPostByKeywords for all users have been fetched and saved.`
      );
  } catch (error) {
